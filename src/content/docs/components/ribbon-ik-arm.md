...
- **Legacy type-string history.** RibbonIKArm was formerly the type `'IKArm'`; that string was vacated for the new base IKArm, and pre-rename `'IKArm'` scene/blueprint data is migrated on load to RibbonIKArm (version-gated). A current `'IKArm'` is the base arm, not this one.
